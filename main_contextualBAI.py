...
mu = np.array([[0.5, 0.45, 0.43, 0.4, 0.2]]).T
...
print("Optimal weights:", optWeights)
print("Optimal weights:", semiparaoptWeights.reshape((K, D)))
...
policies = [contextual_track_stop]
...
def SaveData(mu, delta, num_similator):
    ...
    for imeth in range(lP):
        ...
        for j in range(num_similator):
            """
            mu_context = np.array(
                [np.random.uniform(0.2, 0.8, size=D) for i in range(K)])
            mu_context_x = mu_context*zeta.T
            mu_context *= mu/np.sum(mu_context_x, axis=1).reshape(K, 1)
            """
            recommendation, num_sampling, score_list, recom_arm_list = policy(
                mu_context, zeta, delta)
        ...
        np.savetxt("results/contextual_draws_track_stop60.csv",
                   draws, delimiter=",")
        np.savetxt("results/contextual_recommend_track_stop56.csv",
                   recommend, delimiter=",")
        np.savetxt("results/contextual_recommens_array60.csv",
                   recommends_save, delimiter=",")
        np.savetxt("results/contextual_scores_array60.csv",
